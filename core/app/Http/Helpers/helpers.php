...
function convertCurrency($amount, $fromCurrency)
{
    ...
    if (!isset($currencies['data'][$fromCurrency])) {
        throw new \Exception("Currency {$fromCurrency} is not found in the rates file.");
    }

    $general = GeneralSetting::first();
    $toCurrency = $general->cur_text;
    if (!isset($currencies['data'][$toCurrency])) {
        throw new \Exception("Currency {$toCurrency} is not found in the rates file.");
    }

    // Get the conversion rates
    $fromRate = $currencies['data'][$fromCurrency]['value'];
    $toRate = $currencies['data'][$toCurrency]['value'];

    // Convert the amount to USD first (if it's not already)
...
function getCashback($click, $orderAmount)
{
