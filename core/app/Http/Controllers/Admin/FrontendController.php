...

{
...

    {
     ...
     public function banners(){
        ...
        return view('admin.frontend.banners', compact('pageTitle','banners'));
     }
     ...
     public function saveBanner(Request $request, $id = 0){
        $imgValidation = $id ? 'nullable' : 'required';
         ...
         if ($request->hasFile('image')) {
             try {
                 $banner->img = fileUploader($request->image, getFilePath('banner'), null, $oldImage);
             } catch (\Exception$e) {
         ...
         $notify[] = ['success', "Banner $notification successfully"];
         return back()->withNotify($notify);
     }
     ...
     public function deleteBanner($id){
        try {
            ...
            $notify[] = ['error', 'Error deleting the banner. Please try again.'];
        }
